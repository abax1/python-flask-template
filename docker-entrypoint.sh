...
# Check that a .idam_secret_key file or JWT_SECRET_KEY envvar is set
if [ ! -f .did_secret_key ] && [ -z "$JWT_SECRET_KEY" ]; then
    if [ $WORKERS -gt 1 ]; then
        echo "[ ERROR ] You are configured to use more than 1 worker."
        echo "[ ERROR ] To do this, you must define the JWT_SECRET_KEY environment variable or create a .idam_secret_key file."
        echo "[ ERROR ] Exiting..."
        exit 1
    fi
fi
